...
export async function authenticateBackendSystem() {
  ...
  try {
    ...
    if (
      ...
      console.log("System account not matching or not found. Attempting registration...");
      try {
        const auth = getRawAuth();
      ...
      console.error("❌ Exception authenticating backend system user:", err);
      throw err;
    }
  }
}
...
export class CompatTransaction {
  ...
  async get(ref: CompatDocumentReference) {
